...
class TestGossipingSgd(object):
    @staticmethod
    def gossiping_sgd_check(rank, data, architecture, args):
        logger = logging.getLogger('test_logger.rank_{}'.format(rank))
        trainer = GossipingSGDPullTrainer(
        ...
        for peer_params, param, update in \
            ...
            exp = (param.data - peer_params[trainer.peer]) / 2
    ...
    def test_gossiping_sgd(self, mock_args, sample_data,
